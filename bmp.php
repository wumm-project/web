...
where { ?a a od:BusinessModelPattern . ?b a od:BMP-Dimension .}';
    ...
    foreach($graph->allOfType('od:BusinessModelPattern') as $bmp) {
        ...
        $dim=array();
        foreach ($bmp->all("od:hasDimension") as $v) {
            $dim[]=$v->get("skos:prefLabel");
        }
        ...
        if (!empty($dim)) {
            $out.="<strong>Affected BM Components</strong>: ".join(", ",$dim)."</p>";
        }
        if (!empty($comment)) {
